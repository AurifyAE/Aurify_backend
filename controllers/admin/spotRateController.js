...
export const updateCommodity = async (req, res, next) => {
  try {
    ...
    if (!updatedSpotRate) {
      return res
        .status(404)
        .json({ message: "SpotRate or commodity not found" });
    }
...
export const deleteSpotRateCommodity = async (req, res, next) => {
  try {
    ...
    if (result.nModified === 0) {
      return res
        .status(404)
        .json({ message: "Commodity not found or already deleted" });
    }
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the commodity" });
  }
};
